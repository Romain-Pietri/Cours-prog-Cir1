...
Maillon *NewLinkedListItem(int value) {
    Maillon *tmp;
...
int setvalue(Liste *list, Maillon *item, int value) {
    if(list==NULL || item==NULL) return 0;
    item->data = value;
    return 1;  
}
int getvalue(Liste *l, Maillon *m, int *v){
    if(m==NULL || l==NULL ) return 0;
    *v=m->data;
    return 1;
}

Maillon * getfromvalue(Liste *l, Maillon * start, int value, int sens){
...
int insert_item_index_value(Liste *l,int index, int value){
...
int main(){
    ...
    Myliste = NewListe();
    for( int i=0; i<10;++i){
        inserttail(Myliste, NewLinkedListItem(i));
    }
    insert_after_elem(Myliste,Myliste->head , NewLinkedListItem(42));
    insert_before_elem(Myliste,Myliste->tail , NewLinkedListItem(-42));
    //emptyListe(Myliste);
    Removefromliste(Myliste, Myliste->tail->previous->previous);
    printf(" Debug size : %d | %d\n",sizeofListe(Myliste) ,Myliste->size);
    printf(" Affichage dans l'ordre : ");
    DisplayListe(Myliste);
    printf(" Affichage dans l'ordre inverse : ");
    RevDisplayListe(Myliste);
